...
//把发布状态赋值给数据模型
  articleModel.value.state = clickState;
    if(clickState=="草稿"){
      ...
      ElMessage.success(result.msg ? result.msg : '已添加致审核')
    }else{
      //调用接口
...
const deleteArticle = async (row) => {
    ...
    ElMessageBox.confirm(
        ...
        .then(async () => {
            //调用接口
          if(row.state=="草稿"){
            let result = await DraftDeleteService(row.id);
          }else{
    ...
    <el-card class="page-container">
        <template #header>
            <div class="header">
                <span>文章管理</span>
                <div class="extra">
                    <el-button type="primary" @click="visibleDrawer = true; title = '添加文章'; clear();">添加文章</el-button>
                </div>
            </div>
        </template>
        <!-- 搜索表单 -->
